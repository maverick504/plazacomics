  ...
  <div>
    <form class="pt-no pb-no" @submit.prevent="save" @keydown="form.onKeydown($event)">
        ...
        <div class="column col-6 col-sm-12">
          ...
          <div class="form-group" :class="{ 'has-error': form.errors.has('state') }">
            ...
            <select class="form-select" v-model="form.state">
              ...
              <option value="draft">{{ $t('serie_state_draft') }}</option>
              <option value="active">{{ $t('serie_state_active') }}</option>
              <option value="paused">{{ $t('serie_state_paused') }}</option>
              <option value="canceled">{{ $t('serie_state_canceled') }}</option>
              <option value="completed">{{ $t('serie_state_completed') }}</option>
            </select>
            <p class="form-input-hint">{{ form.errors.get('state') }}</p>
          </div>
        </div>
      </div>
      <!-- Slug -->
      ...
      <div class="form-group">
        ...
        <p class="form-input-hint">{{ form.errors.get('explicit_content') }}</p>
      </div>
      <!-- Licence -->
      ...
      <div class="form-group mt-lg">
        <v-button class="mr-sm" :loading="form.busy" type="primary">
          Actualizar
        </v-button>
      </div>
    </form>
...
export default {
  ...
  created () {
    ...
    this.form.keys().forEach(key => {
      ...
      else if((key=='genre1' || key=='genre2') && this.serie[key]!==null) {
